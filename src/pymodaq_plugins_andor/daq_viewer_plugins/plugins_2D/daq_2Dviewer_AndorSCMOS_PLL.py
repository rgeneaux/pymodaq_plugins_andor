...
class DAQ_2DViewer_AndorSCMOS_PLL(DAQ_Viewer_base):
    """
    """
    ...
    params = comon_parameters + [
        ...
        {'title': 'Timing', 'name': 'timing_opts', 'type': 'group', 'children':
            [{'title': 'Exposure Time (ms)', 'name': 'exposure_time', 'type': 'float', 'value': 1.0},
    ...
    def commit_settings(self, param: Parameter):
        ...
        if param.name() in iter_children(self.settings.child('roi'), []):
            ...
            new_roi = (x0, width, xbin, y0, height, ybin)
            self.update_rois(new_roi)
    ...
    def emit_data(self):
        ...
        try:
            ...
            # (_, unread,_,_) = self.controller.get_frames_status()
            # print(f'{unread} unread frames')
            # frame = self.controller.read_multiple_images()
            do_emit = False
            ...
            if self.live and self.data_shape == 'Data1D':
                ...
                # while self.n_grabed_frames < 1000:
                #     if self.controller.get_new_images_range() is not None:
                #         frames, info = self.controller.read_multiple_images(return_info=True)
                #         self.n_grabed_frames += len(frames)
                #         print(self.n_grabed_frames)
                #         if len(frames) > 1:
                #             self.data += np.mean(frames, axis=0)
                #         elif len(frames) == 1:
                #             self.data += np.squeeze(frames)
